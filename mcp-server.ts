  ...
  async () => {
    ...
    return {
      content: [{
        type: "text",
        text: JSON.stringify({
          intro: "Each block has `id` (unique string), `type` (one of the types below), `props` (type-specific), and optional `children` (for section/columns only). Any block may also carry `box` — `{ paddingY, paddingX, marginTop, marginBottom, borderWidth, radius }` in px, `borderStyle` (solid, dashed, dotted), `borderColor` and `background` as CSS colours, `shadow` (none, sm, md, lg, xl) — for spacing, a border, a shadow and a fill; use it rather than a spacer block. Any block may also carry `motion` — fade, rise, left, right or zoom — to come into view as a visitor scrolls to it. A section's `anchor` names it, and a link to `#c-<anchor>` (or `#<anchor>` typed on a button) scrolls to it. A section, and each entry of a columns block's `columnStyles`, may carry `backgroundGradient` — `{ from, to, angle }`, two CSS colours and degrees (180 runs top to bottom) — drawn instead of `background` when there is no image. Wherever a colour is taken, `var(--site-color-N, #hex)` (N from 1 to 6) names the site's palette colour N, with the hex drawn if the site has none, and `var(--site-accent, #hex)` the accent; a block given one follows the site when its colours change. A prop outside what a block accepts is repaired to a default rather than refused, so use the values in `allowedValues` where a block lists them.",
          blocks: ref,
        }, null, 2),
      }],
    };
