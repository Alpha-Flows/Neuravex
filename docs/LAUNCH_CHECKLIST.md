...
- ☐ **Blocker** — `npm run desktop` rebuilds when the source has changed. *Today:* `ensureBuilt()` (`electron/server.js:57-64`) builds only when `.next` is absent, so after `git pull` the stale bundle runs against a database `firstRun()` has just migrated; `INSTALL.md:538` promises otherwise. *Do:* record a source fingerprint next to the schema fingerprint in `prisma/.neuravex-state.json` and rebuild when it moves.
...
- ☐ **Should** — Running `npm run test:e2e` cannot touch a customer's own sites. *Today:* `playwright.config.ts:40` sets `reuseExistingServer: true` on port 3939 — the launcher's own port — with no `DATABASE_URL` override, so the suite runs against `prisma/dev.db` and fires 95 permanent deletes at it across 18 specs.
...
- ☐ **Should** — Published sites and the builder have their own 404 and error pages. *Today:* no `error.tsx`, `not-found.tsx` or `global-error.tsx` anywhere under `src/app`; visitors get Next's stock page, and a throw in either root layout has nothing above it.
