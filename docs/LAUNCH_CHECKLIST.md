...
| 3 | Next.js is two patches behind a public denial-of-service fix, and the 14.x line has ended (NVX-010, NVX-004) | ☐ | One `npm i next@14.2.35` today; the major upgrade before the app is exposed anywhere, and before it is offered to Windows users on a network. |
...
- ☐ **Blocker** — `next` is at 14.2.35 today and the 15.5.x or 16.x migration is scheduled and gated by the full test suite. *Today:* 14.2.33; the RSC deserialisation DoS is reachable, and the Windows RCE has no 14.x fix. *Do:* patch bump now; major upgrade before any network-reachable or Windows deployment; meanwhile `images.unoptimized: true`, no `sharp`, and a line in `INSTALL.md` that Windows hosts stay loopback-only. (NVX-010, NVX-004)
...
- ☐ **Should** — CI runs `npm audit --omit=dev --audit-level=high` with a dated exception list, and Dependabot or Renovate is configured. *Today:* no audit step, no bot, 13 open advisories. (NVX-065)
...
- ☐ **Nice** — ESLint goes beyond `next/core-web-vitals` (type-aware rules, `no-floating-promises`, `eslint-plugin-no-unsanitized` for the `dangerouslySetInnerHTML` sites), and the move away from `next lint` is planned before the Next upgrade removes it.
