...
| 4 | A link written by an import, a paste or the MCP server runs as script on the customer's exported site (NVX-001), and the same content can lock the owner out of the editor (NVX-045) | ☑ | Done. `isSafeHref()` guards every write path and the render, exported pages carry `EXPORT_CSP` (`src/lib/static-export.ts:295`), and the sanitiser prefixes every `id` so no content can name itself `__next_f`. |
...
- ☑ **Blocker** — The editor sanitises stored HTML before it touches the DOM, and sanitised content cannot name an element `__next_f`. *Today:* done. `src/components/blocks/Editable.tsx:71,84,94` sanitises on write, on read-back and at render; `src/lib/sanitize.ts:40,120-124` prefixes every `id`; the server-side walker at `src/lib/block-tree.ts:169-170` covers content that never passes through the editor. (NVX-024, NVX-045)
...
- ☑ **Should** — Colour, length and background props are validated before they become inline styles, on write and at render. *Today:* done. `colorProp`/`lengthProp` (`src/lib/block-tree.ts:118-120`, via `src/lib/css-value.ts`) on write, `safeProps` at render. (NVX-007)
...
- ☑ **Should** — The exported form cannot submit anywhere its author did not name, and the legal audit scans every string prop and counts links as links. *Today:* done. `disableExportedForms()` (`src/lib/static-export.ts:368`) switches off every form without a destination; one given a form service's https address or an email address (`src/lib/form-fields.ts:201`) posts there, and `injectExportCsp()` (`src/lib/static-export.ts:309`) opens `form-action` for that origin alone. The audit names a form service as where answers go (`form-destination`), never as content the page loads; the audit walks rich-text props (`src/lib/legal/audit.ts:252`) and reports a link as `outbound-link` rather than as third-party content (`:364`). (NVX-020, NVX-021, NVX-056, NVX-057)
...
- ☑ **Nice** — The upload route answers 400, not 500, to a non-file part or a non-multipart body; a malformed percent-encoded asset reference no longer breaks the whole download. *Today:* done — `src/app/api/upload/route.ts:87,157,162` and `src/lib/static-export.ts:257-263`. (NVX-047, NVX-041)
...
- ☑ **Should** — The Node.js policy is one statement everywhere: `engines`, `.nvmrc`, README, INSTALL, CI. *Today:* done, at 22.12 — `package.json:6-8`, `.nvmrc`, `README.md:47`, `INSTALL.md:27-41`, and CI reads `.nvmrc` through `node-version-file` (`ci.yml:29`) so it cannot drift. (NVX-028)
...
- ☑ **Should** — The save endpoint refuses a `content` that is not a block array, and every writer rejects malformed trees. *Today:* done — `normalizeBlockTree` refuses a non-array (`src/lib/block-tree.ts:771`) and the save route applies it (`src/app/api/pages/[id]/save/route.ts:78`). (NVX-014)
...
- ☐ **Should** — One port story. *Today:* Partly: the MCP server's `get_site_url` now hands agents 3939 (`mcp-server.ts:574`) and `setup-mcp.sh:144` agrees. `README.md:66,68` and INSTALL's dev-server sections still say 3000. (NVX-038)
...
- ☐ **Should** — Known limitations state the scope decisions plainly. *Today:* Partly: `README.md:224` has a limitations list. It omits one user / no accounts, German-only legal pages, that forms in a downloaded site cannot submit anywhere, and that the builder targets desktop browsers.
...
- ☐ **Should** — README states the legal feature is German-law only, is not legal advice, and produces German text. *Today:* `README.md:31` describes the feature without any of the three.
...
- ☐ **Nice** — Non-Latin site names get a usable slug instead of `untitled`; uploads prompt for a description; images are resized on upload; the download carries a canonical URL and sitemap once a base URL setting exists, and `og:image` stops pointing at `localhost`. *Today:* Partly: `og:image` and the canonical URL are fixed (`src/lib/static-export.ts:219,239`, NVX-051). A non-Latin name still slugs to `untitled` (`src/lib/utils.ts:44`), and there is no description prompt or resize.
