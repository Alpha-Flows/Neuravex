...
- ☑ **Blocker** — The editor sanitises stored HTML before it touches the DOM, and sanitised content cannot name an element `__next_f`. *Today:* done. `src/components/blocks/Editable.tsx:71,84,94` sanitises on write, on read-back and at render; `src/lib/sanitize.ts:40,120-124` prefixes every `id`; the server-side walker at `src/lib/block-tree.ts:181-182` covers content that never passes through the editor. (NVX-024, NVX-045)
...
- ☑ **Should** — Colour, length and background props are validated before they become inline styles, on write and at render. *Today:* done. `colorProp`/`lengthProp` (`src/lib/block-tree.ts:120-132`, via `src/lib/css-value.ts`) on write, `safeProps` at render. (NVX-007)
- ☑ **Should** — The site accent is validated on every write path and guarded at its three inline sinks. *Today:* done. `safeAccent` (`src/lib/site-fields.ts:42`) guards the dashboard gradient (`src/app/(builder)/page.tsx:66`), the admin swatch (`src/app/(builder)/admin/sites/[id]/page.tsx:37`) and the public logo square (`src/components/public/SiteChrome.tsx:183`). (NVX-008)
...
- ☑ **Should** — Site import runs the same field validators as the settings API. *Today:* done. `src/lib/site-archive.ts:152` calls `normalizeSiteFields()`. (NVX-027)
...
- ☑ **Should** — The public page selects only the columns it renders and passes small objects to client components. *Today:* done. The column select at `src/app/(published)/sites/[siteSlug]/[[...pageSlug]]/page.tsx:81` excludes the legal profile, and `:122-128` passes narrowed objects to the client. (NVX-S001)
...
- ☑ **Should** — Site import normalises page slugs, `isHome` and `legalKind` the way every other writer does. *Today:* done. `pageCreateData()` (`src/lib/site-archive.ts:189`) slugifies, and `normalizeArchivePages()` (`:222`) settles `isHome`, `legalKind` and slug collisions. (NVX-S002)
...
- ☑ **Should** — The Node.js policy is one statement everywhere: `engines`, `.nvmrc`, README, INSTALL, CI. *Today:* done, at 22.12 — `package.json:6-8`, `.nvmrc`, `README.md:51`, `INSTALL.md:27-41`, and CI reads `.nvmrc` through `node-version-file` (`ci.yml:29`) so it cannot drift. (NVX-028)
...
- ☑ **Should** — The save endpoint refuses a `content` that is not a block array, and every writer rejects malformed trees. *Today:* done — `normalizeBlockTree` refuses a non-array (`src/lib/block-tree.ts:787`) and the save route applies it (`src/app/api/pages/[id]/save/route.ts:78`). (NVX-014)
...
- ☐ **Should** — One port story. *Today:* Partly: the MCP server's `get_site_url` now hands agents 3939 (`mcp-server.ts:574`) and `setup-mcp.sh:144` agrees. `README.md:70,72` and INSTALL's dev-server sections still say 3000. (NVX-038)
...
- ☐ **Should** — README's project layout and data model describe the tree that exists. *Today:* Partly: the "DOMPurify" error is gone (`README.md:111` names `sanitize-html`). The layout still uses pre-route-group paths (`:93-117`), the data model lists two of seven models (`:120-122`), and there is no mention of `src/proxy.ts`, `mcp-server.ts`, `scripts/`, `electron/`, `e2e/` or `src/lib/legal/`.
...
- ☑ **Should** — In-app copy does not recommend something the app strips. *Today:* done — the Theme tab no longer sends anyone to Google Fonts. Its fonts are picked from the sixteen bundled under `public/fonts/` or uploaded (`src/components/admin/SiteSettings.tsx:202-204`), and the note beneath says they travel with a downloaded site (`:205-209`).
...
- ☐ **Should** — Known limitations state the scope decisions plainly. *Today:* Partly: `README.md:238` has a limitations list. It omits one user / no accounts, German-only legal pages, that forms in a downloaded site cannot submit anywhere, and that the builder targets desktop browsers.
...
- ☐ **Should** — README states the legal feature is German-law only, is not legal advice, and produces German text. *Today:* `README.md:35` describes the feature without any of the three.
...
- ☐ **Nice** — The delete-site button moves to a "Danger zone" away from the Save row. *Today:* still in the settings body (`src/components/admin/SiteSettings.tsx:326`).
