...
- ☑ **Should** — Site import runs the same field validators as the settings API. *Today:* done. `src/lib/site-archive.ts:186` calls `normalizeSiteFields()`. (NVX-027)
...
- ☑ **Should** — MCP tool results mark stored site text as data, and destructive tools need an explicit confirmation. *Today:* done. `siteData()` (`mcp-server.ts:156`) wraps every result with a data marker; `:319` and `:544` require `confirm: true`. (NVX-S003)
- ☑ **Should** — Site import normalises page slugs, `isHome` and `legalKind` the way every other writer does. *Today:* done. `pageCreateData()` (`src/lib/site-archive.ts:235`) slugifies, and `normalizeArchivePages()` (`:283`) settles `isHome`, `legalKind` and slug collisions. (NVX-S002)
...
- ☑ **Should** — The Node.js policy is one statement everywhere: `engines`, `.nvmrc`, README, INSTALL, CI. *Today:* done, at 22.12 — `package.json:6-8`, `.nvmrc`, `README.md:63`, `INSTALL.md:27-41`, and CI reads `.nvmrc` through `node-version-file` (`ci.yml:29`) so it cannot drift. (NVX-028)
- ☐ **Should** — The version string has one source. *Today:* Partly: both launchers read it from `package.json` (`electron/server.js:18`, `mcp-server.ts:182`), so the three-way duplication is gone. It is still not shown in the admin footer, so a customer cannot say what they are running.
...
- ☑ **Should** — The save endpoint refuses a `content` that is not a block array, and every writer rejects malformed trees. *Today:* done — `normalizeBlockTree` refuses a non-array (`src/lib/block-tree.ts:819`) and the save route applies it (`src/app/api/pages/[id]/save/route.ts:86`). (NVX-014)
...
- ☐ **Should** — One port story. *Today:* Partly: the MCP server's `get_site_url` now hands agents 3939 (`mcp-server.ts:577`) and `setup-mcp.sh:144` agrees. `README.md:82,84` and INSTALL's dev-server sections still say 3000. (NVX-038)
...
- ☐ **Should** — README's project layout and data model describe the tree that exists. *Today:* Partly: the "DOMPurify" error is gone (`README.md:123` names `sanitize-html`). The layout still uses pre-route-group paths (`:105-129`), the data model lists two of seven models (`:132-134`), and there is no mention of `src/proxy.ts`, `mcp-server.ts`, `scripts/`, `electron/`, `e2e/` or `src/lib/legal/`.
...
- ☐ **Should** — Known limitations state the scope decisions plainly. *Today:* Partly: `README.md:254` has a limitations list. It omits one user / no accounts, German-only legal pages, that forms in a downloaded site cannot submit anywhere, and that the builder targets desktop browsers.
...
- ☐ **Should** — README states the legal feature is German-law only, is not legal advice, and produces German text. *Today:* `README.md:47` describes the feature without any of the three.
