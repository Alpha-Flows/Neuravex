...
- ☑ **Blocker** — The editor sanitises stored HTML before it touches the DOM, and sanitised content cannot name an element `__next_f`. *Today:* done. `src/components/blocks/Editable.tsx:71,84,94` sanitises on write, on read-back and at render; `src/lib/sanitize.ts:40,120-124` prefixes every `id`; the server-side walker at `src/lib/block-tree.ts:184-185` covers content that never passes through the editor. (NVX-024, NVX-045)
...
- ☑ **Should** — The iframe allowlist matches exact hostnames from one shared module that the CSP also uses. *Today:* done. `src/lib/embed-hosts.ts:22` holds exact hosts; the CSP derives from it (`src/proxy.ts:140`) and so does the sanitiser (`src/lib/sanitize.ts:196`). (NVX-023)
...
- ☑ **Should** — Site import runs the same field validators as the settings API. *Today:* done. `src/lib/site-archive.ts:181` calls `normalizeSiteFields()`. (NVX-027)
- ☑ **Should** — Custom header and footer placeholders escape the values they substitute. *Today:* done. `src/components/public/SiteChrome.tsx:89-93` escapes, and `:107` replaces via a function so `$&`-style patterns are inert. (NVX-062)
...
- ☑ **Should** — The public page selects only the columns it renders and passes small objects to client components. *Today:* done. The column select at `src/components/public/PublishedPage.tsx:39`, shared by the page route and the 404 boundary, excludes the legal profile, and `:106-116` passes narrowed objects to the client. (NVX-S001)
...
- ☑ **Should** — Site import normalises page slugs, `isHome` and `legalKind` the way every other writer does. *Today:* done. `pageCreateData()` (`src/lib/site-archive.ts:227`) slugifies, and `normalizeArchivePages()` (`:270`) settles `isHome`, `legalKind` and slug collisions. (NVX-S002)
...
- ☑ **Nice** — The upload route answers 400, not 500, to a non-file part or a non-multipart body; a malformed percent-encoded asset reference no longer breaks the whole download. *Today:* done — `src/app/api/upload/route.ts:87,157,162` and `src/lib/static-export.ts:264-270`. (NVX-047, NVX-041)
- ☑ **Nice** — `data:` links are allowed on images only and every `target="_blank"` carries `rel`. *Today:* done — `src/lib/sanitize.ts:137-142`, and `rel="noopener noreferrer"` at `:146` and `:198`. (NVX-048, NVX-070)
...
- ☑ **Should** — The Node.js policy is one statement everywhere: `engines`, `.nvmrc`, README, INSTALL, CI. *Today:* done, at 22.12 — `package.json:6-8`, `.nvmrc`, `README.md:58`, `INSTALL.md:27-41`, and CI reads `.nvmrc` through `node-version-file` (`ci.yml:29`) so it cannot drift. (NVX-028)
...
- ☑ **Should** — The save endpoint refuses a `content` that is not a block array, and every writer rejects malformed trees. *Today:* done — `normalizeBlockTree` refuses a non-array (`src/lib/block-tree.ts:806`) and the save route applies it (`src/app/api/pages/[id]/save/route.ts:94`). (NVX-014)
...
- ☐ **Should** — One port story. *Today:* Partly: the MCP server's `get_site_url` now hands agents 3939 (`mcp-server.ts:574`) and `setup-mcp.sh:144` agrees. `README.md:77,79` and INSTALL's dev-server sections still say 3000. (NVX-038)
...
- ☐ **Should** — README's project layout and data model describe the tree that exists. *Today:* Partly: the "DOMPurify" error is gone (`README.md:118` names `sanitize-html`). The layout still uses pre-route-group paths (`:100-124`), the data model lists two of seven models (`:127-129`), and there is no mention of `src/proxy.ts`, `mcp-server.ts`, `scripts/`, `electron/`, `e2e/` or `src/lib/legal/`.
...
- ☑ **Should** — In-app copy does not recommend something the app strips. *Today:* done — the Theme tab no longer sends anyone to Google Fonts. Its fonts are picked from the sixteen bundled under `public/fonts/` or uploaded (`src/components/admin/SiteSettings.tsx:244-246`), and the note beneath says they travel with a downloaded site (`:247-251`).
...
- ☐ **Should** — Known limitations state the scope decisions plainly. *Today:* Partly: `README.md:247` has a limitations list. It omits one user / no accounts, German-only legal pages, that forms in a downloaded site cannot submit anywhere, and that the builder targets desktop browsers.
...
- ☐ **Should** — README states the legal feature is German-law only, is not legal advice, and produces German text. *Today:* `README.md:42` describes the feature without any of the three.
...
- ☐ **Nice** — The delete-site button moves to a "Danger zone" away from the Save row. *Today:* still in the settings body (`src/components/admin/SiteSettings.tsx:384`).
