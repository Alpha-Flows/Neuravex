...
| 4 | A link written by an import, a paste or the MCP server runs as script on the customer's exported site (NVX-001), and the same content can lock the owner out of the editor (NVX-045) | ☑ | Done. `isSafeHref()` guards every write path and the render, exported pages carry `EXPORT_CSP` (`src/lib/static-export.ts:302`), and the sanitiser prefixes every `id` so no content can name itself `__next_f`. |
...
- ☑ **Blocker** — Link and media URLs in block props are validated on every write path and at render, and exported pages carry a meta CSP. *Today:* done. `isSafeHref()` (`src/lib/url-safety.ts:23`) guards button hrefs (`src/lib/block-tree.ts:345`) and media sources (`:77`) on the way in and at render (`src/components/blocks/BlockView.tsx:62`); every exported page carries `EXPORT_CSP` (`src/lib/static-export.ts:302`). (NVX-001)
...
- ☑ **Should** — The site accent is validated on every write path and guarded at its three inline sinks. *Today:* done. `safeAccent` (`src/lib/site-fields.ts:45`) guards the dashboard gradient (`src/app/(builder)/page.tsx:66`), the admin swatch (`src/app/(builder)/admin/sites/[id]/page.tsx:60`) and the public logo square (`src/components/public/SiteChrome.tsx:263`). (NVX-008)
...
- ☑ **Should** — Site import runs the same field validators as the settings API. *Today:* done. `src/lib/site-archive.ts:184` calls `normalizeSiteFields()`. (NVX-027)
...
- ☑ **Should** — The public page selects only the columns it renders and passes small objects to client components. *Today:* done. The column select at `src/components/public/PublishedPage.tsx:42`, shared by the page route and the 404 boundary, excludes the legal profile, and `:111-143` passes narrowed objects to the client. (NVX-S001)
...
- ☑ **Should** — The exported form cannot submit anywhere its author did not name, and the legal audit scans every string prop and counts links as links. *Today:* done. `disableExportedForms()` (`src/lib/static-export.ts:375`) switches off every form without a destination; one given a form service's https address or an email address (`src/lib/form-fields.ts:201`) posts there, and `injectExportCsp()` (`src/lib/static-export.ts:316`) opens `form-action` for that origin alone. The audit names a form service as where answers go (`form-destination`), never as content the page loads; the audit walks rich-text props (`src/lib/legal/audit.ts:252`) and reports a link as `outbound-link` rather than as third-party content (`:364`). (NVX-020, NVX-021, NVX-056, NVX-057)
...
- ☑ **Should** — Site import normalises page slugs, `isHome` and `legalKind` the way every other writer does. *Today:* done. `pageCreateData()` (`src/lib/site-archive.ts:233`) slugifies, and `normalizeArchivePages()` (`:281`) settles `isHome`, `legalKind` and slug collisions. (NVX-S002)
...
- ☑ **Nice** — The upload route answers 400, not 500, to a non-file part or a non-multipart body; a malformed percent-encoded asset reference no longer breaks the whole download. *Today:* done — `src/app/api/upload/route.ts:87,196,201` and `src/lib/static-export.ts:264-270`. (NVX-047, NVX-041)
...
- ☑ **Should** — The Node.js policy is one statement everywhere: `engines`, `.nvmrc`, README, INSTALL, CI. *Today:* done, at 22.12 — `package.json:6-8`, `.nvmrc`, `README.md:61`, `INSTALL.md:27-41`, and CI reads `.nvmrc` through `node-version-file` (`ci.yml:29`) so it cannot drift. (NVX-028)
...
- ☑ **Should** — The save endpoint refuses a `content` that is not a block array, and every writer rejects malformed trees. *Today:* done — `normalizeBlockTree` refuses a non-array (`src/lib/block-tree.ts:818`) and the save route applies it (`src/app/api/pages/[id]/save/route.ts:86`). (NVX-014)
...
- ☐ **Should** — One port story. *Today:* Partly: the MCP server's `get_site_url` now hands agents 3939 (`mcp-server.ts:577`) and `setup-mcp.sh:144` agrees. `README.md:80,82` and INSTALL's dev-server sections still say 3000. (NVX-038)
...
- ☐ **Should** — README's project layout and data model describe the tree that exists. *Today:* Partly: the "DOMPurify" error is gone (`README.md:121` names `sanitize-html`). The layout still uses pre-route-group paths (`:103-127`), the data model lists two of seven models (`:130-132`), and there is no mention of `src/proxy.ts`, `mcp-server.ts`, `scripts/`, `electron/`, `e2e/` or `src/lib/legal/`.
...
- ☑ **Should** — In-app copy does not recommend something the app strips. *Today:* done — the Theme tab no longer sends anyone to Google Fonts. Its fonts are picked from the sixteen bundled under `public/fonts/` or uploaded (`src/components/admin/SiteSettings.tsx:253-255`), and the note beneath says they travel with a downloaded site (`:256-260`).
...
- ☐ **Should** — Known limitations state the scope decisions plainly. *Today:* Partly: `README.md:251` has a limitations list. It omits one user / no accounts, German-only legal pages, that forms in a downloaded site cannot submit anywhere, and that the builder targets desktop browsers.
...
- ☐ **Should** — README states the legal feature is German-law only, is not legal advice, and produces German text. *Today:* `README.md:45` describes the feature without any of the three.
...
- ☐ **Nice** — Non-Latin site names get a usable slug instead of `untitled`; uploads prompt for a description; images are resized on upload; the download carries a canonical URL and sitemap once a base URL setting exists, and `og:image` stops pointing at `localhost`. *Today:* Partly: `og:image` and the canonical URL are fixed (`src/lib/static-export.ts:226,246`, NVX-051). A non-Latin name still slugs to `untitled` (`src/lib/utils.ts:44`), and there is no description prompt or resize.
- ☐ **Nice** — The delete-site button moves to a "Danger zone" away from the Save row. *Today:* still in the settings body (`src/components/admin/SiteSettings.tsx:393`).
