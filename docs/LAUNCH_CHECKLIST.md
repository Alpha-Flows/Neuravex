...
| --- | --- | --- | --- |
| 1 | Uploads made while the app runs answer 404 until restart (NVX-005) | ☑ | Done. `/uploads/*` is a route handler (`src/app/uploads/[name]/route.ts:52`) reading from outside `public/`, and the browser suite now runs against `next start`, so the case is covered rather than invisible. |
...
| 4 | A link written by an import, a paste or the MCP server runs as script on the customer's exported site (NVX-001), and the same content can lock the owner out of the editor (NVX-045) | ☑ | Done. `isSafeHref()` guards every write path and the render, exported pages carry `EXPORT_CSP` (`src/lib/static-export.ts:304`), and the sanitiser prefixes every `id` so no content can name itself `__next_f`. |
...
- ☑ **Blocker** — Link and media URLs in block props are validated on every write path and at render, and exported pages carry a meta CSP. *Today:* done. `isSafeHref()` (`src/lib/url-safety.ts:23`) guards button hrefs (`src/lib/block-tree.ts:345`) and media sources (`:77`) on the way in and at render (`src/components/blocks/BlockView.tsx:62`); every exported page carries `EXPORT_CSP` (`src/lib/static-export.ts:304`). (NVX-001)
- ☑ **Blocker** — Request bodies are read with a hard cap before parsing, form submissions are rate- and count-limited, and block trees are schema-, depth- and size-validated on the editor, PATCH, import, paste and MCP write paths. *Today:* done for all of those. `src/lib/request-body.ts:54` streams with a cap; `src/app/api/submissions/route.ts:23` rate-limits and caps per page; `src/lib/block-tree.ts:60-65` bounds depth, node count, bytes and `sortOrder`; `src/lib/css-safety.ts:125` scans CSS iteratively. (NVX-016, NVX-017, NVX-018, NVX-013, NVX-014, NVX-015, NVX-019)
- ☑ **Blocker** — *Every* route that reads a body uses the cap, and saved blocks and revision restore go through the same block-tree schema as the rest. *Today:* done. No route in `src/app/api` calls `req.json()` any more; the last three read through `src/lib/request-body.ts`. Saved blocks go through `normalizeBlockTree` on the way in (`src/app/api/saved-blocks/route.ts`) and again on the way out (`src/components/editor/SavedBlocks.tsx`), because a row kept by an earlier version was never checked and the picker is where one becomes part of a page — the same argument `pasteable()` makes about the clipboard. The legal routes keep an absent body distinct from an empty one, so generating from what is on file still works. The upload is written to disk as it arrives rather than read into memory, against a limit for each kind of file — 250 MB for a video, 10 MB for a picture or a document (`src/lib/media-kind.ts:92`, `src/app/api/upload/route.ts:85`). Next copies every body the proxy sees and cuts it off at 10 MB, so that one route is left out of the proxy (`src/proxy.ts:257`) and runs its `gate()` itself (`src/proxy.ts:185`); `src/proxy.test.ts` fails if any other route leaves the proxy or one that does skips the gate. Every other route's limit fits inside Next's (`src/lib/request-body.ts:29`).
- ☑ **Blocker** — The unused image optimizer is switched off. *Today:* done. `next.config.js:39` sets `unoptimized: true` and `remotePatterns` is gone, so `/_next/image` fetches nothing on request. (NVX-012)
- ☑ **Blocker** — The editor sanitises stored HTML before it touches the DOM, and sanitised content cannot name an element `__next_f`. *Today:* done. `src/components/blocks/Editable.tsx:71,84,94` sanitises on write, on read-back and at render; `src/lib/sanitize.ts:40,120-124` prefixes every `id`; the server-side walker at `src/lib/block-tree.ts:185-186` covers content that never passes through the editor. (NVX-024, NVX-045)

### Should

- ☑ **Should** — `style` attributes are filtered through the existing CSS declaration checker. *Today:* done. `src/lib/sanitize.ts:106` routes them through `sanitizeStyleAttribute`, and `src/lib/css-safety.ts:227` blocks `position`, `z-index` and `pointer-events` so an overlay cannot be built. The same overlay could be built from the builder's own Tailwind classes — `fixed inset-0 z-50` — so `sanitizeClassAttribute` (`src/lib/css-safety.ts:295`, applied at `src/lib/sanitize.ts:115`) refuses positioning, stacking and negative margins written as a `class` too. (NVX-006)
- ☑ **Should** — Colour, length and background props are validated before they become inline styles, on write and at render. *Today:* done. `colorProp`/`lengthProp` (`src/lib/block-tree.ts:124-136`, via `src/lib/css-value.ts`) on write, `safeProps` at render. (NVX-007)
- ☑ **Should** — The site accent is validated on every write path and guarded at its three inline sinks. *Today:* done. `safeAccent` (`src/lib/site-fields.ts:47`) guards the dashboard gradient (`src/app/(builder)/page.tsx:70`), the admin swatch (`src/app/(builder)/admin/sites/[id]/page.tsx:67`) and the public logo square (`src/components/public/SiteChrome.tsx:263`). (NVX-008)
...
- ☑ **Should** — The iframe allowlist matches exact hostnames from one shared module that the CSP also uses. *Today:* done. `src/lib/embed-hosts.ts:22` holds exact hosts; the CSP derives from it (`src/proxy.ts:140`) and so does the sanitiser (`src/lib/sanitize.ts:198`). (NVX-023)
- ☑ **Should** — The SVG sanitiser resolves attribute local names before checking URLs. *Today:* done. `splitAttribute()` (`src/lib/security.ts:176`) resolves the prefix, applied at `:245-249`. (NVX-025)
- ☑ **Should** — Site import runs the same field validators as the settings API. *Today:* done. `src/lib/site-archive.ts:190` calls `normalizeSiteFields()`. (NVX-027)
- ☑ **Should** — Custom header and footer placeholders escape the values they substitute. *Today:* done. `src/components/public/SiteChrome.tsx:108-112` escapes, and `:126` replaces via a function so `$&`-style patterns are inert. (NVX-062)
...
- ☑ **Should** — The `X-Forwarded-Host` header is only trusted when the operator says there is a proxy, and the nginx example sets it explicitly. *Today:* done. `trustsProxy()` (`src/proxy.ts:82`) gates it behind `NEURAVEX_TRUST_PROXY`, `servingHost()` (`:181`) applies it, and the recipe sets the header (`INSTALL.md:283`). (NVX-034, NVX-036)
- ☑ **Should** — The public page selects only the columns it renders and passes small objects to client components. *Today:* done. The column select at `src/components/public/PublishedPage.tsx:48`, shared by the page route and the 404 boundary, excludes the legal profile, and `:120-152` passes narrowed objects to the client. (NVX-S001)
...
- ☑ **Should** — Uploads are checked against their magic bytes, stripped of EXIF metadata, and served through a route that refuses symlinks. *Today:* done. Magic bytes `src/lib/uploads.ts:156`, EXIF stripping `:220` (both used by `src/app/api/upload/route.ts:138,244,249`), and the serving route refuses symlinks via `lstatSync` (`src/lib/uploads.ts:89`) and opens the file without following one (`src/app/uploads/[name]/route.ts:72`). (NVX-061, NVX-059, NVX-054)
- ☑ **Should** — The exported form cannot submit anywhere its author did not name, and the legal audit scans every string prop and counts links as links. *Today:* done. `disableExportedForms()` (`src/lib/static-export.ts:377`) switches off every form without a destination; one given a form service's https address or an email address (`src/lib/form-fields.ts:201`) posts there, and `injectExportCsp()` (`src/lib/static-export.ts:318`) opens `form-action` for that origin alone. The audit names a form service as where answers go (`form-destination`), never as content the page loads; the audit walks rich-text props (`src/lib/legal/audit.ts:252`) and reports a link as `outbound-link` rather than as third-party content (`:364`). (NVX-020, NVX-021, NVX-056, NVX-057)
- ☑ **Should** — MCP tool results mark stored site text as data, and destructive tools need an explicit confirmation. *Today:* done. `siteData()` (`mcp-server.ts:157`) wraps every result with a data marker; `:320` and `:559` require `confirm: true`. (NVX-S003)
- ☑ **Should** — Site import normalises page slugs, `isHome` and `legalKind` the way every other writer does. *Today:* done. `pageCreateData()` (`src/lib/site-archive.ts:239`) slugifies, and `normalizeArchivePages()` (`:287`) settles `isHome`, `legalKind` and slug collisions. (NVX-S002)
...
- ☑ **Nice** — `X-Powered-By` is suppressed, `Cross-Origin-Opener-Policy` and `Cross-Origin-Resource-Policy` are set, DNS prefetch is off, and the proxy recipe adds HSTS. *Today:* done — `next.config.js:6,66,73,78` and `INSTALL.md:263`. (NVX-046, NVX-073, NVX-036)
...
- ☑ **Nice** — The upload route answers 400, not 500, to a non-file part or a non-multipart body; a malformed percent-encoded asset reference no longer breaks the whole download. *Today:* done — `src/app/api/upload/route.ts:87,196,201` and `src/lib/static-export.ts:266-272`. (NVX-047, NVX-041)
- ☑ **Nice** — `data:` links are allowed on images only and every `target="_blank"` carries `rel`. *Today:* done — `src/lib/sanitize.ts:137-142`, and `rel="noopener noreferrer"` at `:185` and `:251`. (NVX-048, NVX-070)
- ☑ **Nice** — JSON API responses send `Cache-Control: no-store` so a caching proxy cannot store exports or submissions. *Today:* done — `next.config.js:82-88` on `/api/:path*`. (NVX-068)
...
- ☑ **Nice** — `sourceMappingURL` comments are stripped in `sanitizeCss()` as insurance against a future postcss downgrade. *Today:* done — `src/lib/security.ts:53`. (NVX-064)

---

## 2. First run, packaging and releases

- ☑ **Blocker** — A picture uploaded during a session is served immediately in production mode. *Today:* done. Uploads live outside `public/` (`src/lib/uploads.ts:29`) and are served by a route handler (`src/app/uploads/[name]/route.ts:52`), so Next's one-time snapshot of `public/` no longer decides what a visitor can fetch; `e2e/production-only.spec.ts:19` covers it against `next start`. (NVX-005)
...
- ☑ **Should** — The Node.js policy is one statement everywhere: `engines`, `.nvmrc`, README, INSTALL, CI. *Today:* done, at 22.12 — `package.json:6-8`, `.nvmrc`, `README.md:67`, `INSTALL.md:27-41`, and CI reads `.nvmrc` through `node-version-file` (`ci.yml:29`) so it cannot drift. (NVX-028)
- ☐ **Should** — The version string has one source. *Today:* Partly: both launchers read it from `package.json` (`electron/server.js:18`, `mcp-server.ts:183`), so the three-way duplication is gone. It is still not shown in the admin footer, so a customer cannot say what they are running.
...
- ☑ **Blocker** — The unit baseline is green and reproducible: 36 files, 635 tests, `tsc` clean, lint clean (0 errors and 0 warnings), a fresh `next build` with no warnings, lockfile v3 with `npm ci`.
...
- ☐ **Should** — The headline features each have an end-to-end test. *Today:* Partly: `e2e/trash.spec.ts` covers restore and unpublish, and `e2e/page-work.spec.ts` covers reordering by button and keyboard (`:101`) and a revision restored through the panel and the restore undone (`:195`). Still missing: drag from the palette and drag to reorder, page duplicate/move/make-home, redo, and the MCP tools over stdio.
...
- ☑ **Should** — The JSON export is described honestly and can be imported from the UI. *Today:* done. "Import a site" sits beside "New site" (`src/app/(builder)/page.tsx:39`, `src/components/admin/ImportSiteButton.tsx:12`) and takes a JSON export or a backup; the dashboard's "Back up this site" is a zip of the archive and every upload it names, with each file's library name and description (`src/app/api/sites/[id]/backup/route.ts:24`), brought back through the upload route file by file (`src/lib/site-import.ts:21`). The JSON link beside it says it carries the pages and settings alone.
...
- ☑ **Nice** — Restoring a revision cancels the pending autosave and snapshots the pre-restore content, so a restore is itself reversible. *Today:* done. The editor stops the autosave timer and saves the canvas before the restore is sent, or keeps it as a version of its own when it cannot be saved (`src/components/editor/PageEditor.tsx:457`, called from `src/components/editor/RevisionsPanel.tsx:67`); the restore keeps the page as a named version first and returns its id (`src/lib/revisions.ts:132`), which the editor offers as "Undo the restore". A stray autosave landing afterwards is refused by the version check. Driven in `e2e/page-work.spec.ts:195`.
...
- ☐ **Should** — Logs survive a closed window and swallowed errors say something. *Today:* Partly: the launcher's own output is on the console. There is still no log file, and `src/app/api/upload/route.ts:22` still swallows a failed `mediaFile.create`.
...
- ☐ **Should** — One port story. *Today:* Partly: the MCP server's `get_site_url` now hands agents 3939 (`mcp-server.ts:592`) and `setup-mcp.sh:144` agrees. `README.md:86,88` and INSTALL's dev-server sections still say 3000. (NVX-038)
...
- ☐ **Should** — README's project layout and data model describe the tree that exists. *Today:* Partly: the "DOMPurify" error is gone (`README.md:127` names `sanitize-html`). The layout still uses pre-route-group paths (`:109-133`), the data model lists two of seven models (`:136-138`), and there is no mention of `src/proxy.ts`, `mcp-server.ts`, `scripts/`, `electron/`, `e2e/` or `src/lib/legal/`.
...
- ☑ **Should** — In-app copy does not recommend something the app strips. *Today:* done — the Theme tab no longer sends anyone to Google Fonts. Its fonts are picked from the sixteen bundled under `public/fonts/` or uploaded (`src/components/admin/SiteSettings.tsx:261-263`), and the note beneath says they travel with a downloaded site (`:264-268`).
...
- ☐ **Should** — A user guide beyond README: block reference, template list (generated from `TEMPLATES`), keyboard shortcuts (the editor implements Cmd+Z, Shift+Cmd+Z, Cmd+C/X/V/D, Delete, Escape, Ctrl+B/I, Alt+↑/↓, and Shift- or Cmd/Ctrl-click to choose several blocks; README documents Cmd+S), the legal-pages flow. *Today:* none.
...
- ☐ **Should** — Known limitations state the scope decisions plainly. *Today:* Partly: `README.md:258` has a limitations list. It omits one user / no accounts, German-only legal pages, that forms in a downloaded site cannot submit anywhere, and that the builder targets desktop browsers.
...
- ☑ **Should** — Nothing in the builder or the published pages loads external fonts, images or scripts; templates reference `/stock/` only; visitor submissions store no IP, user agent or referrer; nothing about the operator is stored or sent. Verified. The site's fonts are the builder's own files or the site's uploads, and nothing else can be named in a `@font-face` rule (`src/lib/fonts.ts:103`, `:127`).
...
- ☐ **Should** — Keyboard-only users can reorder blocks and reach every control. *Today:* Partly. Every block in the flow has move up and down buttons on its toolbar (`src/components/blocks/Sortable.tsx:213`) and Alt+↑/↓ moves the chosen block (`src/components/editor/PageEditor.tsx:868`), both driven in `e2e/page-work.spec.ts:101`. Still open: dragging is the only way into another container short of the inspector's container picker, and modals do not trap or return focus.
...
- ☐ **Nice** — Large-site performance. *Today:* Partly: the media picker no longer rescans every page (`src/app/api/media/route.ts:26`, NVX-042) and export CSS is cached (`src/lib/export-css.ts:26`, NVX-043). Still open: the pages list loads full `content` just to show titles, and the revisions panel refetches up to 50 full snapshots, and every named version up to a hundred, after every autosave (`src/app/api/pages/[id]/revisions/route.ts:25`).
- ☐ **Nice** — Non-Latin site names get a usable slug instead of `untitled`; uploads prompt for a description; images are resized on upload; the download carries a canonical URL and sitemap once a base URL setting exists, and `og:image` stops pointing at `localhost`. *Today:* Partly. `og:image` and the canonical URL are fixed (`src/lib/static-export.ts:228,248`, NVX-051); pictures are resized and given smaller copies in the browser (`src/lib/image-prep.ts:78`); and with the site's address set in Settings → SEO the download writes the head's addresses in full (`src/app/api/sites/[id]/download/route.ts:147`) and carries a `sitemap.xml` (`:232`). A non-Latin name still slugs to `untitled` (`src/lib/utils.ts:44`), and an upload does not ask for a description — the check before publishing lists the pictures without one.
- ☐ **Nice** — The delete-site button moves to a "Danger zone" away from the Save row. *Today:* still in the settings body (`src/components/admin/SiteSettings.tsx:438`).
